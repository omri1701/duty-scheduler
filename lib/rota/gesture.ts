export type HoldDragOptions={enabled:boolean;onStart:(key:string)=>void;onMove:(key:string)=>void;onEnd:(key:string|null)=>void;onCancel?:()=>void};
export function attachHoldDrag(root:HTMLDivElement,getOptions:()=>HoldDragOptions,ignoreUntil:{current:number}){
  ...
  function start(x:number,y:number,touch:boolean,id:number){if(!getOptions().enabled)return;const key=at(x,y);if(!key)return;g={key,x,y,active:false,touch,last:key,id};if(touch)timer=setTimeout(activate,360);}
  function move(x:number,y:number,e:Event){if(!g)return;const moved=Math.hypot(x-g.x,y-g.y);if(!g.active){if(g.touch&&moved>10){cancel();return}if(!g.touch&&moved>6)activate();}if(g?.active){if(e.cancelable)e.preventDefault();const key=at(x,y);g.last=key;if(key)getOptions().onMove(key);}}
